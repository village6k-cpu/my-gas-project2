...
function onOpen() {
  ...
  ui.createMenu("📋 빌리지 스케줄")
    ...
    .addItem("🔄 계약서 재생성", "regenerateContractFromMenu")
    .addSeparator()
    .addItem("🔄 장비 목록 갱신", "refreshEquipmentList")
    .addItem("📸 실사기록 동기화", "syncAuditFromMaster")
    .addSeparator()
    .addItem("🗑️ 등록완료 건 정리", "clearCompletedRequests")
    .addSeparator()
...
function regenerateContractFromMenu() {
  ...
  var 거래ID;
  if (sheet.getName() === "스케줄상세") {
    거래ID = sheet.getRange(row, 2).getValue();  // B열
  } else if (sheet.getName() === "계약마스터") {
    거래ID = sheet.getRange(row, 1).getValue();  // A열
  } else if (sheet.getName() === "확인요청") {
    거래ID = sheet.getRange(row, 16).getValue(); // P열
  } else {
    var response = ui.prompt("거래ID를 입력하세요:");
  ...
  try {
    var result = deleteAndRegenerateContract(ss, String(거래ID).trim());
    ui.alert("✅ 계약서 재생성 완료!\n\n거래ID: " + 거래ID + "\n링크: " + result.url);
  } catch (err) {
    ui.alert("❌ 계약서 재생성 실패:\n" + err.message);
  }
...
function checkSingleRowWithData(sheet, row, reqID, 반출일, 반출시간, 반납일, 반납시간,
  ...
  // ── 시간대별 피크 사용량 계산 (동시 최대 사용량 기준) ──
  // 요청 기간과 겹치는 기존 예약들을 모은 뒤,
  // 각 예약의 시작/끝 시점을 기준으로 시간대별 동시 사용량의 최대값을 구한다.
  // 예) 이성준 4/6 10-22시 2대, 이규직 4/7 11-24시 2대 → 동시 최대 2대 (겹치지 않으므로)
  //     김태완 4/6 20시~4/8 20시 요청 → 보유4 - 피크2 = 가용2 ✅
  ...
  schedData.forEach(sched => {
    ...
    if (schedStart < reqEndDT && schedEnd > reqStartDT) {
      겹치는예약.push({ start: schedStart, end: schedEnd, qty: sched.qty, name: sched.contractName || sched.contractID });
      ...
      const overlapStr = overlapMin >= 60 ? `${Math.floor(overlapMin/60)}시간 ${overlapMin%60}분` : `${overlapMin}분`;
      겹침목록.push(`${sched.contractName || sched.contractID} 반납${endStr}(${overlapStr} 겹침)`);
    }
  });

  // 시간대별 피크 계산: 각 예약의 시작/끝 시점을 이벤트로 만들어 스위핑
  let 피크사용 = 0;
  if (겹치는예약.length > 0) {
    // 이벤트 포인트 수집 (요청 기간 내로 클리핑)
    let events = [];
    겹치는예약.forEach(r => {
      const clippedStart = r.start < reqStartDT ? reqStartDT : r.start;
      const clippedEnd = r.end > reqEndDT ? reqEndDT : r.end;
      events.push({ time: clippedStart.getTime(), delta: +r.qty });
      events.push({ time: clippedEnd.getTime(), delta: -r.qty });
    });
    // 시간순 정렬 (같은 시각이면 종료(-) 먼저 처리 → 반납 후 반출은 안 겹침)
    events.sort((a, b) => a.time - b.time || a.delta - b.delta);
    let current = 0;
    events.forEach(ev => {
      current += ev.delta;
      if (current > 피크사용) 피크사용 = current;
    });
  }
  ...
  if (가용 >= 수량) {
    result = `✅ 가용${가용}`;
    detail = `보유${총보유}` + (피크사용 > 0 ? `, 최대동시${피크사용}` : "");
  } else if (가용 > 0 && 가용 < 수량) {
    result = `⚠️ 부족(가용${가용}/${수량})`;
    detail = `보유${총보유}, 최대동시${피크사용}`;
...
/**
 * 등록완료 건 수동 정리 (메뉴에서 실행)
 * 등록완료/거절/보류된 건만 삭제. 나머지는 그대로.
 */
function clearCompletedRequests() {
  ...
  const sheet = ss.getSheetByName("확인요청");
  const lastRow = sheet.getLastRow();
  if (lastRow < 2) { ui.alert("정리할 건이 없습니다."); return; }
  ...
  var count = 0;
  for (let i = 0; i < data.length; i++) {
    var status = String(data[i][14]).trim();  // O열
    if (status.indexOf("등록완료") >= 0 || status === "거절" || status === "보류") count++;
  }

  if (count === 0) { ui.alert("정리할 등록완료/거절/보류 건이 없습니다."); return; }

  var response = ui.alert("확인요청 정리", "등록완료/거절/보류 " + count + "건을 정리하시겠습니까?", ui.ButtonSet.YES_NO);
  if (response !== ui.Button.YES) return;

  // 아래에서 위로 삭제
  for (let i = data.length - 1; i >= 0; i--) {
    var status = String(data[i][14]).trim();
    if (status.indexOf("등록완료") >= 0 || status === "거절" || status === "보류") {
      var row = i + 2;
      sheet.getRange(row, 1, 1, 11).clearContent();
      sheet.getRange(row, 1, 1, 11).setBackground(null).setFontWeight("normal");
      sheet.getRange(row, 13, 1, 6).clearContent();  // M~R열
      sheet.getRange(row, 13, 1, 6).setBackground(null).setFontWeight("normal");
    }
  }

  ui.alert("✅ " + count + "건 정리 완료");
}

/**
 * 기존 자동 초기화 트리거 제거 (1회 실행)
 */
function removeAutoClearTrigger() {
  ...
  Logger.log("autoClearRequests 트리거 " + removed + "개 삭제됨");
  try {
    SpreadsheetApp.getUi().alert("✅ 자동 초기화 트리거 " + removed + "개 삭제 완료");
  } catch(e) {}
}
