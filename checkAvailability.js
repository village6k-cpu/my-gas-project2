...
function getInventoryConflicts(hoursAhead) {
  var report=getInventoryRiskReport();
  var end=Number(hoursAhead)>0 ? Date.parse(report.coverage.start)+Number(hoursAhead)*3600000 : Infinity;
  var alerts=report.alerts.filter(function(a){return !a.start || Date.parse(a.start)<end;});
  function when(value){return Utilities.formatDate(new Date(value),'Asia/Seoul','M/d HH:mm');}
  var conflicts=alerts.filter(function(a){return a.kind==='shortage';}).map(function(a){return {
    장비명:a.equipment,총보유:a.totalStock,가용재고:a.stock,최대동시사용:a.booked,부족수량:a.shortage,충돌시점:when(a.start),
    예약들:a.bookings.map(function(b){return {거래ID:b.tradeId,예약자명:b.customer,반출:when(b.start),반납:when(b.end),수량:b.quantity};})};});
  return {windowStart:report.coverage.start,windowEnd:Number.isFinite(end)?new Date(end).toISOString():report.coverage.end,
    coverage:report.coverage,conflictCount:conflicts.length,conflicts:conflicts,riskCount:alerts.filter(function(a){return a.kind!=='shortage';}).length,
    alerts:alerts};
}

/** Same all-future, time-aware report as the monitor and the staff operations view. */
function getInventoryConflictsSlackMessage() {
  var report=getInventoryRiskReport();
  return inventoryRiskSlackText_(report,{changed:report.alerts,resolved:0},inventoryRiskDetailUrl_());
}
...
// 2026-07-31 배포가 만든 사진 전체 인덱스. 사진 수에 따라 커지며 반출 상태와
// mutation log가 함께 쓰는 500KB Script Properties를 소진하므로 정리 전용으로만 남긴다.
var DASHBOARD_RETIRED_PHOTO_PROPERTY_MANIFEST_KEY_ = 'dashboard_photo_file_index_v2_property_manifest';
var DASHBOARD_RETIRED_PHOTO_PROPERTY_CHUNK_PREFIX_ = 'dashboard_photo_file_index_v2_property_';

function cleanupRetiredDashboardPhotoIndexProperties_(props) {
  try {
    var cache = CacheService.getScriptCache();
    if (cache.get('dashboardRetiredPhotoPropertyCleanup_v1')) return 0;
    props = props || PropertiesService.getScriptProperties();
    var removed = 0;
    props.getKeys().forEach(function(key) {
      if (
        key === DASHBOARD_RETIRED_PHOTO_PROPERTY_MANIFEST_KEY_ ||
        key.indexOf(DASHBOARD_RETIRED_PHOTO_PROPERTY_CHUNK_PREFIX_) === 0
      ) {
        props.deleteProperty(key);
        removed++;
      }
    });
    cache.put('dashboardRetiredPhotoPropertyCleanup_v1', '1', 21600);
    return removed;
  } catch (err) {
    return 0;
  }
...
function toggleItemChecksBatch(tid, entries) {
  ...
  var props = PropertiesService.getScriptProperties();
  // 사진 인덱스가 Script Properties 총량을 소진해도 실제 품목 mutation log를 쓰기 전에
  // 폐기된 대용량 조각을 제거하므로, 현장 반출 체크가 즉시 자가 복구된다.
  cleanupRetiredDashboardPhotoIndexProperties_(props);
...
function toggleItemCheck(scheduleId, phase, done, options) {
  ...
  var isDone = done === true || done === "true" || done === "1" || done === 1;
  var mutationId = normalizeDashboardMutationId_(options && options.mutationId);
  if (phase === 'checkout') cleanupRetiredDashboardPhotoIndexProperties_(props);
...
// CacheService는 키당 100KB 제한이라 사진 인덱스를 여러 캐시 항목으로 나눈다.
...
function resolveDashboardPhotoFileId_(value, fileIndex) {
  ...
  if (fileId) {
    ...
    // AppSheet가 파일을 추가한 경우도 첫 단건 해석 후 현재 6시간 캐시에 합쳐,
    // 같은 캐시 수명 안의 다른 기기가 Drive 단건 조회를 반복하지 않게 한다.
...
function rememberDashboardPhotoFileIndexEntry_(fileName, fileId) {
  ...
  try {
    ...
    var index = readDashboardPhotoFileIndexCache_(cache);
    if (!index) return;
...
/**
 * 완료된 거래의 레거시 중복 행을 안전하게 정리하기 위한 검증 계획.
 * keep/remove 두 행의 B:M 값이 모두 같고, 더 늦은 suffix/행만 remove일 때만 허용한다.
 * 일반 품목 삭제와 달리 반출·반납 완료 증거는 다시 열지 않는다.
 */
function buildDashboardDuplicateRepairPlan_(tid, pairs, scheduleRows) {
  tid = String(tid || '').trim();
  if (!/^\d{6}-\d{3}$/.test(tid)) return { ok: false, error: '올바른 거래ID 필요' };
  if (typeof pairs === 'string') {
    try { pairs = JSON.parse(pairs); } catch (parseErr) { return { ok: false, error: 'pairs JSON 형식 오류' }; }
  }
  if (!Array.isArray(pairs) || !pairs.length || pairs.length > 30) {
    return { ok: false, error: 'keepId/removeId 쌍 1~30개 필요' };
  }

  var rowsById = {};
  (scheduleRows || []).forEach(function(row, index) {
    var scheduleId = String((row && row[0]) || '').trim();
    if (!scheduleId || String((row && row[1]) || '').trim() !== tid) return;
    rowsById[scheduleId] = { row: row, sheetRow: index + 2 };
  });

  var used = {};
  var planned = [];
  for (var i = 0; i < pairs.length; i++) {
    var keepId = String((pairs[i] && pairs[i].keepId) || '').trim();
    var removeId = String((pairs[i] && pairs[i].removeId) || '').trim();
    var idPattern = new RegExp('^' + tid.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '-(\\d+)$');
    var keepMatch = keepId.match(idPattern);
    var removeMatch = removeId.match(idPattern);
    if (!keepMatch || !removeMatch || keepId === removeId) {
      return { ok: false, error: '거래ID에 속한 서로 다른 keepId/removeId가 필요합니다' };
    }
    if (used[keepId] || used[removeId]) return { ok: false, error: '중복 지정된 스케줄ID가 있습니다' };
    used[keepId] = true;
    used[removeId] = true;
    if (Number(removeMatch[1]) <= Number(keepMatch[1])) {
      return { ok: false, error: 'removeId는 keepId보다 나중 suffix여야 합니다: ' + removeId };
    }

    var keep = rowsById[keepId];
    var remove = rowsById[removeId];
    if (!keep || !remove) return { ok: false, error: '원본 시트에서 keep/remove 행을 모두 찾지 못했습니다' };
    if (remove.sheetRow <= keep.sheetRow) return { ok: false, error: 'remove 행은 keep 행보다 뒤에 있어야 합니다' };

    var keepFingerprint = JSON.stringify((keep.row || []).slice(1, 13).map(function(v) { return String(v == null ? '' : v).trim(); }));
    var removeFingerprint = JSON.stringify((remove.row || []).slice(1, 13).map(function(v) { return String(v == null ? '' : v).trim(); }));
    if (keepFingerprint !== removeFingerprint) {
      return { ok: false, error: '행 내용이 완전히 같지 않아 삭제를 차단했습니다: ' + removeId };
    }
    planned.push({
      keepId: keepId,
      removeId: removeId,
      keepRow: keep.sheetRow,
      removeRow: remove.sheetRow,
      name: String(remove.row[3] || '').trim(),
      setName: String(remove.row[2] || '').trim(),
      qty: Number(remove.row[4]) || 1
    });
  }
  ...
  return {
    ...
    tradeId: tid,
    pairs: planned,
    removeIds: planned.map(function(row) { return row.removeId; }),
    removeRows: planned.map(function(row) { return row.removeRow; })
  };
}

/**
 * 정확히 검증된 레거시 중복 행만 원본 스케줄상세에서 제거한다.
 * 기본값은 dry-run. 반출/반납 완료와 계약 상태는 보존하고 제거 ID의 낡은 체크 속성만 지운다.
 */
function repairDashboardDuplicateScheduleRows(tid, pairs, options) {
  options = options || {};
  var execute = options.dryRun === false || options.dryRun === 0 || options.dryRun === '0' || options.dryRun === 'false';
  var lock = null;
  if (execute) {
  ...
  try {
    var ss = SpreadsheetApp.getActiveSpreadsheet();
    var sheet = ss.getSheetByName('스케줄상세');
    if (!sheet || sheet.getLastRow() < 2) return { error: '스케줄상세 비어있음' };
    var rows = sheet.getRange(2, 1, sheet.getLastRow() - 1, 13).getDisplayValues();
    var plan = buildDashboardDuplicateRepairPlan_(tid, pairs, rows);
    if (!plan.ok) return { error: plan.error, dryRun: !execute };
    if (!execute) {
      return {
        success: true,
        dryRun: true,
    ...
    deleteDashboardRowsDescending_(sheet, plan.removeRows);
    var props = PropertiesService.getScriptProperties();
    plan.removeIds.forEach(function(scheduleId) {
      props.deleteProperty('itemCheck_' + scheduleId + '_checkout');
      props.deleteProperty('itemCheck_' + scheduleId + '_checkin');
      props.deleteProperty('itemCheckQty_' + scheduleId + '_checkin');
      props.deleteProperty('itemCheckProof_' + scheduleId + '_checkin');
    });
    try { supaMarkTradeDirty_(tid); } catch (dirtyErr) {}
    try { invalidateDashboardCacheForTrade_(tid); } catch (cacheErr) {}
    ...
    return {
      ...
      dryRun: false,
      tradeId: plan.tradeId,
      removedRows: plan.removeRows.length,
      removedScheduleIds: plan.removeIds,
      preservedCompletion: true
    };
  } finally {
...
function _sanitizeConfirmRequestFreeText_(value, maxLength) {
  ...
  raw.split(/\n+/).forEach(function(line) {
    ...
    if (!line) return;
    if (internalPattern.test(line)) return;
...
function _isMutableConfirmRequestGroup_(group) {
  ...
  return !/(등록완료|등록대기|등록\s*처리|개고생2\.0|거절|보류|취소)/.test(statusText) &&
    !(group.registerActions || []).some(function(action) { return String(action).trim() === "등록"; });
}
 ...
 */
function _resolveStaffConfirmedPendingRequestFence_(sheet, fence, options) {
  ...
  if ((!allowIncompleteExpectedPeriod && (!currentPeriod.start_date || !currentPeriod.start_time || !currentPeriod.end_date || !currentPeriod.end_time)) ||
...
// Source evidence is bound to the immutable conversation by the trusted caller.
// GAS validates its shape and rechecks the actual customer/RQ baseline under lock.
function _assertInquirySourceEvidence_(evidence) {
  if (!evidence || typeof evidence !== "object" || Array.isArray(evidence) ||
      typeof evidence.customer_request !== "string" || !evidence.customer_request.trim() ||
      !String(evidence.conversation_revision || "").trim() ||
      !/^[a-f0-9]{64}$/i.test(String(evidence.conversation_evidence_hash || "")) ||
      !Array.isArray(evidence.customer_message_ids) || !evidence.customer_message_ids.length ||
...
function _normalizeCustomerRequestedPendingBaselinePeriod_(period) {
  ...
  if (!period || typeof period !== "object" || Array.isArray(period) || Object.keys(period).length !== fields.length) {
    throw new Error("고객 문의 수정 baseline period는 공란을 포함한 네 필드를 명시해야 합니다.");
  }
  var normalized = {};
  fields.forEach(function(field) {
    if (!Object.prototype.hasOwnProperty.call(period, field) || typeof period[field] !== "string") {
      throw new Error("고객 문의 수정 baseline period의 누락 필드는 추측하지 않습니다: " + field);
    }
    var value = period[field].trim();
    if (value && (field.indexOf("date") >= 0 ? !_confirmRequestStrictDate_(value) : !/^([01]\d|2[0-3]):00$/.test(value))) {
      throw new Error("고객 문의 수정 baseline period 형식 오류: " + field);
    }
...
function _resolveCustomerRequestedPendingRevision_(sheet, req) {
  var fence = req.customer_requested_pending_revision;
  if (fence === undefined || fence === null) return null;
  if (req.staff_confirmed_pending_mutation || req.staff_confirmed_registration_bootstrap ||
      req.inquiry_disposition === "independent_rental") {
    throw new Error("고객 문의 수정과 다른 실행 권한을 함께 지정할 수 없습니다.");
  }
...
function _assertNoUnfencedPendingRevision_(sheet, req, requestedEquipItems) {
  var startDate = _confirmRequestDateKey_(req.반출일);
  if (!startDate || !requestedEquipItems.length) return;
  var incoming = [startDate, _confirmRequestTimeKey_(req.반출시간),
    _confirmRequestDateKey_(req.반납일), _confirmRequestTimeKey_(req.반납시간)];
  var matches = _buildConfirmRequestGroups_(sheet).filter(function(group) {
    if (!_isMutableConfirmRequestGroup_(group) ||
        !_confirmRequestSameIdentity_(group, String(req.예약자명 || "").trim(), _confirmRequestPhoneKey_(req.연락처)) ||
        group.startDate !== startDate ||
        _confirmRequestEquipListEquivalent_(group.topLevelEquipItems, requestedEquipItems)) return false;
    var existing = [group.startDate, group.startTime, group.endDate, group.endTime];
    return incoming.every(function(value, index) { return !value || !existing[index] || value === existing[index]; });
  });
  if (!matches.length) return;
  var error = new Error("같은 고객·기간의 대기 문의가 있어 exact 고객 요청 수정 계약이 필요합니다.");
  error.code = "pending_revision_required";
  error.existingRequestIds = matches.map(function(group) { return group.reqID; });
  if (matches.length === 1) error.existingRequestId = matches[0].reqID;
  throw error;
}
...
function _insertAndCheckRequest(req) {
  ...
  var customerPendingFence = req.customer_requested_pending_revision
    ? _resolveCustomerRequestedPendingRevision_(sheet, req) : null;
  if (customerPendingFence) {
    staffConfirmedPendingFence = customerPendingFence;
    ...
    req.staff_confirmed_exact_set_components = _projectStaffConfirmedDesiredSetComponents_(
      customerPendingFence.expectedBefore, customerPendingFence.expectedSetComponents,
      requestedEquipItems.map(function(item) { return { name: item.name, quantity: Number(item.qty) }; }),
      ss.getSheetByName("세트마스터"), []
    );
  }
  var independentRental = req.inquiry_disposition === "independent_rental";
  if (independentRental) _assertInquirySourceEvidence_(req.inquiry_source_evidence);
  ...
  var customerDbMatches = confirmedRegistrationBootstrap || customerPendingFence
    ? []
  ...
  var resolvedDiscount = confirmedRegistrationBootstrap || customerPendingFence
  ...
  // Current registered inventory is authoritative for repeated whole/subset plans.
  // A real new item/quantity still creates an inquiry for the staff mutation path.
  if (!staffConfirmedPendingFence && !independentRental && !confirmedRegistrationBootstrap) {
    var registeredReceipt = _reconcileRegisteredConfirmRequest_(ss, reqForDedupe, requestedEquipItems);
    if (registeredReceipt) return registeredReceipt;
  }
  if (!staffConfirmedPendingFence && !confirmedRegistrationBootstrap) {
    _assertNoUnfencedPendingRevision_(sheet, reqForDedupe, requestedEquipItems);
  }
  var registeredTradeId = independentRental ? null : _findRegisteredTradeForConfirmRequest_(ss, reqForDedupe);
  ...
  if (customerPendingFence) {
    var stagedPlanMatches = _confirmRequestEquipListEquivalent_(requestedEquipItems,
      finalTopLevelPlan.map(function(item) { return {name: item.name, qty: item.quantity}; }));
    var stagedPeriodMatches = finalPeriod && finalPeriod.start_date === req.반출일 &&
      finalPeriod.start_time === req.반출시간 && finalPeriod.end_date === req.반납일 && finalPeriod.end_time === req.반납시간;
    if (!stagedPlanMatches || !stagedPeriodMatches ||
        !_staffConfirmedSetComponentsEquivalent_(finalSetComponents, req.staff_confirmed_exact_set_components)) {
      var stagedMismatch = new Error("고객 문의 수정의 새 장비·기간·세트 readback이 계획과 다릅니다.");
      stagedMismatch.code = "pending_revision_readback_mismatch";
      throw stagedMismatch;
    }
  }
  var response = {
    reqID: reqID,
  ...
  if (replacedReqIDs.length > 0) {
    ...
    var cutoverFence = customerPendingFence
      ? _resolveCustomerRequestedPendingRevision_(sheet, req)
      : _resolveStaffConfirmedPendingRequestFence_(sheet, req.staff_confirmed_pending_mutation);
  ...
  if (staffConfirmedPendingFence) {
    ...
    if (customerPendingFence) {
      response.customer_requested_pending_revision = response.staff_confirmed_pending_mutation;
      delete response.staff_confirmed_pending_mutation;
    }
  }
  return response;
  ...
  if (finalLastRow2 >= 2) {
    ...
    for (let i = 0; i < finalData2.length; i++) {
      ...
      if (fResult) {
        ...
        if (fResult.indexOf("모델 선택 필요") >= 0) sheet.getRange(fRow, 6).setBackground("#FFEB9C");
      }
    }
  }
 ...
 */
function _availabilityEquipNameKey_(value) {
...
function findEquipmentForSetHeader_(name, equipSheet) {
  ...
  if (!equipSheet) return null;

  var targetKey = _availabilityEquipNameKey_(name);
  if (targetKey.length < 4) return null;
  var lastRow = equipSheet.getLastRow();
  if (lastRow < 2) return null;
  var data = equipSheet.getRange(2, 1, lastRow - 1, 12).getValues();
  var candidates = [];
  for (var i = 0; i < data.length; i++) {
    var candidateName = String(data[i][3] || "").trim();
    var candidateKey = _availabilityEquipNameKey_(candidateName);
    if (!candidateKey) continue;
    if (candidateKey.indexOf(targetKey) >= 0 || targetKey.indexOf(candidateKey) >= 0) {
      candidates.push({ name: candidateName, total: data[i][4] || 0, 단가: data[i][11] || 0 });
    }
  }
...
function checkSetHeaderAvail(sheet, row, 장비명, 수량, 반출일, 반출시간, 반납일, 반납시간, schedData, equipSheet) {
  ...
  var overlaps = [];
  var setNameKey = _availabilityEquipNameKey_(장비명);
  var inventoryNameKey = _availabilityEquipNameKey_(equipInfo.name);
  schedData.forEach(function(sched) {
    var schedNameKey = _availabilityEquipNameKey_(sched.equipment);
    if (schedNameKey !== setNameKey && schedNameKey !== inventoryNameKey) return;
...
function _modelSelectionCandidateNames_(categoryItems, maxCount) {
  ...
  var seen = {};
  maxCount = maxCount || 8;
  for (var ci = 0; ci < (categoryItems || []).length && names.length < maxCount; ci++) {
    var name = String(categoryItems[ci].name || categoryItems[ci].장비명 || categoryItems[ci] || "").trim();
...
function _setModelSelectionPrompt_(sheet, row, equipName, categoryItems, ownerSet) {
  var names = _modelSelectionCandidateNames_(categoryItems, 8);
  var detail = "F열에서 구체 모델 선택 필요";
  if (ownerSet) detail += " (세트: " + ownerSet + ")";
  if (names.length > 0) detail += " — 후보: " + names.join(" / ");
  if ((categoryItems || []).length > names.length) detail += " 외 " + ((categoryItems || []).length - names.length) + "개";
  ...
  try {
    sheet.getRange(row, 6).setNote(
      '⚠️ 구체적인 모델을 선택하세요\n' +
      (ownerSet ? '세트: ' + ownerSet + '\n' : '') +
      '현재값: ' + String(equipName || '') + '\n' +
      '─────────────\n' +
...
function refreshModelSelectionPrompts() {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var sheet = ss.getSheetByName("확인요청");
  var equipSheet = ss.getSheetByName("장비마스터");
  if (!sheet || !equipSheet) return { success: false, error: "확인요청/장비마스터 시트 없음" };
  var lastRow = sheet.getLastRow();
  if (lastRow < 2) return { success: true, updated: 0, rows: [] };
  var data = sheet.getRange(2, 1, lastRow - 1, 18).getValues();
  var updated = [];
  for (var i = 0; i < data.length; i++) {
    var result = String(data[i][8] || "").trim();
    var detail = String(data[i][9] || "").trim();
    if (result.indexOf("모델 선택 필요") < 0 && detail.indexOf("F열 드롭다운에서 구체 모델") < 0) continue;
    var equipName = String(data[i][5] || "").trim();
    if (!equipName) continue;
    if (findEquipment(equipName, equipSheet)) continue;
    var catItems = findEquipmentByCategory(equipName, equipSheet);
    if (!catItems.length) continue;
    var ownerSet = "";
    var qTag = String(data[i][16] || "").trim();
    if (qTag.indexOf("[세트]") === 0) ownerSet = qTag.substring(4);
    var row = i + 2;
    var newDetail = _setModelSelectionPrompt_(sheet, row, equipName, catItems, ownerSet);
    updated.push({ row: row, equipment: equipName, detail: newDetail });
  }
...
function checkSingleRowWithData(sheet, row, reqID, 반출일, 반출시간, 반납일, 반납시간,
  ...
  if (!equipInfo) {
    ...
    if (catItems.length > 0) {
      ...
      _setModelSelectionPrompt_(sheet, row, 장비명, catItems, ownerSet);
      return;
    }
    if (_isSetAccessoryManifestRow_(sheet, row, 장비명)) {
      sheet.getRange(row, 9).setValue("ℹ️ 기본구성");
      sheet.getRange(row, 10).setValue("세트 동봉품(개별 재고 미관리)");
      sheet.getRange(row, 9, 1, 2).setBackground("#D9EAF7");
      return;
    }
...
function _isCompositeSetAccessoryManifest_(name) {
  ...
  var parts = value.split(/[\/／]/).map(function(part) { return part.trim(); }).filter(function(part) { return part; });
  if (parts.length < 2) return false;
  if (parts.length >= 3) return true;
  return /(메모리|배터리|충전기|캡|후드|마이크로\s*sd|블림프|붐폴|xlr|데드캣|d탭|sdi|hdmi|숏암|안테나|플레이트|클램프|그립|케이블|라인|패널|발라스터|루버|실크|디퓨|프레임|케이스|파우치|이어폰|브라켓|어댑터|리플렉터|반도어|고보|아이리스|오링|로드)/i.test(value);
}
 ...
 */
function planMergedScheduleRows_(allData, reqID, mergeTargetTID, existingScheduleRows) {
  function scheduleSpecFromRequest_(row) {
    var name = String((row && row[5]) || '').trim();
    if (!name) return null;
    var status = String((row && row[14]) || '').trim();
    if (status === '거절' || status === '보류' || status === '제외') return null;
    var result = String((row && row[8]) || '').trim();
    var note = String((row && row[16]) || '').trim();
    var setName = result === '세트'
      ? name
      : note.indexOf('[세트]') === 0
        ? note.substring(4).trim()
        : name;
    return { setName: setName, name: name, qty: Number(row[6]) || 1 };
  }
  function signature_(setName, name, qty) {
  ...
  var existingCounts = {};
  if (mergeTargetTID) {
    (existingScheduleRows || []).forEach(function(row) {
      if (String((row && row[1]) || '').trim() !== String(mergeTargetTID).trim()) return;
      if (String((row && row[9]) || '').trim() === '취소') return;
      var sig = signature_(row[2], row[3], row[4]);
      existingCounts[sig] = (existingCounts[sig] || 0) + 1;
    });
  }

  var writeSourceIndexes = [];
  var skippedDuplicateRows = [];
  var explicitlyAdditive = false;
  (allData || []).forEach(function(row, index) {
    if (String((row && row[0]) || '').trim() !== String(reqID || '').trim()) return;
    var extraRequest = String((row && row[17]) || '').trim();
    if (/추가|더/u.test(extraRequest) && !/추가\s*(?:없|불필요)|더\s*(?:없|필요\s*없)/u.test(extraRequest)) {
      explicitlyAdditive = true;
    }
    var spec = scheduleSpecFromRequest_(row);
    if (!spec) return;
    var sig = signature_(spec.setName, spec.name, spec.qty);
    if (mergeTargetTID && existingCounts[sig] > 0) {
      existingCounts[sig]--;
      skippedDuplicateRows.push({
        sourceIndex: index,
    ...
    writeSourceIndexes.push(index);
  });

  // A separate add-on request made entirely of an already-active item is still additive.
  // Treating it as a successful no-op silently loses the customer's extra quantity.
  if (mergeTargetTID && skippedDuplicateRows.length > 0 && (writeSourceIndexes.length === 0 || explicitlyAdditive)) {
    writeSourceIndexes = writeSourceIndexes
      .concat(skippedDuplicateRows.map(function(row) { return row.sourceIndex; }))
...
function registerByReqID(sheet, triggerRow, registerOptions) {
  ...
  // 어느 행/어느 세트 구성품이 막는지 O/I/J/F note에 명시한다.
  const 미선택목록 = [];
  for (let i = 0; i < allData.length; i++) {
    if (allData[i][0] !== reqID) continue;
    ...
    if (!eqCheck) {
      ...
      if (catItems.length > 0) {
        var issueRow = i + 2;
        var ownerSet = "";
        var qTag = String(allData[i][16] || "").trim();
        if (qTag.indexOf("[세트]") === 0) ownerSet = qTag.substring(4);
        var detail = _setModelSelectionPrompt_(sheet, issueRow, 장비명, catItems, ownerSet);
        var label = issueRow + "행 " + (ownerSet ? ownerSet + " > " : "") + "'" + 장비명 + "'";
        var names = _modelSelectionCandidateNames_(catItems, 3);
        if (names.length) label += " (후보: " + names.join(" / ") + (catItems.length > names.length ? " 외" : "") + ")";
        미선택목록.push(label);
      }
    }
  }
  if (미선택목록.length > 0) {
    sheet.getRange(triggerRow, 15).setValue("❌ 모델 미선택: " + 미선택목록.join("; ") + " → 해당 F열에서 구체 모델 선택 후 다시 등록");
    sheet.getRange(triggerRow, 15).setBackground("#FFC7CE");
    sheet.getRange(triggerRow, 14).clearContent();
    return;
  }
      ...
      if (_sLR >= 2) {
        ...
        if (mergeTargetTID) {
          mergeTargetScheduleRows = _sData.filter(function(row) {
            return String(row[1] || '').trim() === String(mergeTargetTID).trim();
          });
        }
      }
  ...
  // 같은 빌리지 등록 경로끼리는 동시에 같은 번호를 발급할 수 없다.
  const now = new Date();
  const dateStr = Utilities.formatDate(now, "Asia/Seoul", "yyMMdd");
  ...
  // 개고생2.0 거래내역도 같은 스프레드시트 권한으로 직접 확인한다.
  try {
    ...
    // 연결 시트 접근 실패 시 기존처럼 계약마스터 번호를 사용한다.
  }
  ...
  contractSheet.getRange(newContractRow, 1, 1, 12).setValues([[
    ...
    if (mergeMode && mergeTargetTID && schedLastRow >= 2) {
      ...
      var neededRows = mergeSchedulePlan.writeSourceIndexes.length;
      if (mergeMode && mergeTargetTID) {
        ...
        if (mergeLastRow > 0 && neededRows > 0) {
    ...
    mergeSchedulePlan.writeSourceIndexes.forEach(function(sourceIndex) {
      const i = sourceIndex;
  ...
  if (mergeMode && mergeTargetTID && mergeSchedulePlan.writeSourceIndexes.length > 0) {
 ...
 */
function _findRegisteredTradesForConfirmRequest_(ss, req, allowPartialPeriod) {
  req = req || {};
  ...
  var reqEndDate = _confirmRequestDateKey_(req.반납일);
  var reqEndTime = _confirmRequestTimeKey_(req.반납시간);
  if ((!reqName && !reqPhone) || !reqStartDate ||
      (!allowPartialPeriod && (!reqStartTime || !reqEndDate || !reqEndTime))) return [];
  ...
  for (var i = 0; i < rows.length; i++) {
    ...
    if (_confirmRequestDateKey_(row[4], displayRow[4]) !== reqStartDate
        || (reqStartTime && _confirmRequestTimeKey_(row[5], displayRow[5]) !== reqStartTime)
        || (reqEndDate && _confirmRequestDateKey_(row[6], displayRow[6]) !== reqEndDate)
        || (reqEndTime && _confirmRequestTimeKey_(row[7], displayRow[7]) !== reqEndTime)) continue;
    matchedTradeIds[tradeId] = true;
  }
  return Object.keys(matchedTradeIds);
}

function _reconcileRegisteredConfirmRequest_(ss, req, requestedEquipItems) {
  var candidates = _findRegisteredTradesForConfirmRequest_(ss, req, true);
  if (candidates.length > 1) {
    var ambiguous = new Error("동일 고객·호환 기간의 등록 거래가 여러 건이라 대상 확인이 필요합니다.");
    ambiguous.code = "registered_reconciliation_ambiguous";
    throw ambiguous;
  }
  if (candidates.length !== 1 || !requestedEquipItems.length) return null;
  var scheduleSheet = ss.getSheetByName("스케줄상세");
  if (!scheduleSheet || scheduleSheet.getLastRow() < 2) return null;
  var range = scheduleSheet.getRange(2, 1, scheduleSheet.getLastRow() - 1, 10);
  var rows = range.getValues();
  var displayRows = range.getDisplayValues();
  var quantities = Object.create(null);
  var requestedQuantities = Object.create(null);
  var results = [];
  var requestedPeriod = [_confirmRequestDateKey_(req.반출일), _confirmRequestTimeKey_(req.반출시간),
    _confirmRequestDateKey_(req.반납일), _confirmRequestTimeKey_(req.반납시간)];
  requestedEquipItems.forEach(function(item) {
    var qty = Number(item.qty);
    if (!Number.isInteger(qty) || qty < 1) throw new Error("문의 장비 수량은 양의 정수여야 합니다.");
    requestedQuantities[item.name] = (requestedQuantities[item.name] || 0) + qty;
  });
  rows.forEach(function(row, index) {
    var display = displayRows[index] || [];
    if (String(display[1] || row[1] || "").trim() !== candidates[0] ||
        /^(취소|삭제|거절)/.test(String(display[9] || row[9] || "").trim())) return;
    var setName = String(display[2] || row[2] || "").trim();
    var name = String(display[3] || row[3] || "").trim();
    if (!name || (setName && setName !== name)) return;
    var period = [_confirmRequestDateKey_(row[5], display[5]), _confirmRequestTimeKey_(row[6], display[6]),
      _confirmRequestDateKey_(row[7], display[7]), _confirmRequestTimeKey_(row[8], display[8])];
    if (!period.every(Boolean) || !requestedPeriod.every(function(value, i) { return !value || value === period[i]; })) return;
    var quantity = Number(row[4] || display[4]);
    if (!Number.isInteger(quantity) || quantity < 1) return;
    quantities[name] = (quantities[name] || 0) + quantity;
    results.push({장비명: name, 수량: quantity, 결과: "등록완료", 상세: "등록 스케줄 확인", scheduleId: String(display[0] || row[0] || ""),
      period: {start_date: period[0], start_time: period[1], end_date: period[2], end_time: period[3]}});
  });
  if (!Object.keys(requestedQuantities).every(function(name) { return (quantities[name] || 0) >= requestedQuantities[name]; })) return null;
  return {alreadyRegistered: true, duplicate: true, matchedRegisteredTradeId: candidates[0],
    results: results, message: "요청 장비·수량은 기존 등록 거래에 이미 반영되어 있습니다."};
}
...
function _buildCheckoutMsg(customerName) {
  return customerName + ' 감독님, 안녕하세요.\n'
    ...
    + '2. 결제\n'
    + '1) 계좌이체: 안내받으신 견적서 상 금액을 [우리은행 1005-404-109661 최재형(빌리지)] 계좌로 이체해주시면 됩니다.(세금계산서/현금영수증 시 반드시 부가세포함가 입금)\n\n'
    + '2) 카드결제: 카운터의 토스 프론트 단말기에서 셀프 카드결제 부탁드립니다.\n'
