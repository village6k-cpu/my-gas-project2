...
function profileDashboardData(targetDate) {
  var today = targetDate || Utilities.formatDate(new Date(), 'Asia/Seoul', 'yyyy-MM-dd');
  var profile = { date: today, steps: [] };
  function step(name, fn) {
    var start = Date.now();
    var value = fn();
    var entry = { name: name, ms: Date.now() - start };
    if (value && value.__count !== undefined) entry.count = value.__count;
    profile.steps.push(entry);
    return value && value.__value !== undefined ? value.__value : value;
  }

  var ss = step('getActiveSpreadsheet', function() { return SpreadsheetApp.getActiveSpreadsheet(); });
  var schedSheet = step('get sheets', function() {
    return {
      sched: ss.getSheetByName('스케줄상세'),
      contract: ss.getSheetByName('계약마스터'),
      set: ss.getSheetByName('세트마스터')
    };
  });
  if (!schedSheet.sched || schedSheet.sched.getLastRow() < 2) return profile;
  var schedLastRow = step('스케줄상세 getLastRow', function() { return schedSheet.sched.getLastRow(); });
  var checkoutRows = step('반출일 TextFinder', function() {
    var rows = findDashboardRowsByValue_(schedSheet.sched, 6, schedLastRow, today);
    return { __value: rows, __count: rows.length };
  });
  var checkinRows = step('반납일 TextFinder', function() {
    var rows = findDashboardRowsByValue_(schedSheet.sched, 8, schedLastRow, today);
    return { __value: rows, __count: rows.length };
  });
  var rowSet = {};
  checkoutRows.forEach(function(row) { rowSet[row] = true; });
  checkinRows.forEach(function(row) { rowSet[row] = true; });
  var todayRows = Object.keys(rowSet).map(function(row) { return Number(row); });
  todayRows.sort(function(a, b) { return a - b; });
  var data = step('오늘 행 display read', function() {
    var rows = readDashboardScheduleRowsDisplay_(schedSheet.sched, todayRows, 12);
    return { __value: rows, __count: rows.length };
  });

  var tradeGroups = {};
  step('tradeGroups build', function() {
    data.forEach(function(row) {
      var 거래ID = String(row[1] || '').trim();
      var 장비명 = row[3];
      var 반출일str = String(row[5] || '').trim();
      var 반납일str = String(row[7] || '').trim();
      var 상태 = row[9] || '대기';
      if (!장비명 || !반출일str || !반납일str || !거래ID || 상태 === '취소') return;
      if (반출일str !== today && 반납일str !== today) return;
      if (!tradeGroups[거래ID]) tradeGroups[거래ID] = { equipments: [] };
      tradeGroups[거래ID].equipments.push({ name: String(장비명 || '').trim() });
    });
    return { __count: Object.keys(tradeGroups).length };
  });
  var tradeIds = Object.keys(tradeGroups);
  step('계약마스터 map', function() { return { __value: getDashboardContractMapForIds_(schedSheet.contract, tradeIds), __count: tradeIds.length }; });
  step('거래내역 extras', function() { return { __value: getTradeExtrasForIds_(tradeIds), __count: tradeIds.length }; });
  step('장비체크 map', function() { return { __value: getEquipmentCheckMapForIds_(tradeIds), __count: tradeIds.length }; });
  step('장비주의사항 rules', function() {
    var rules = getEquipmentRiskRules_();
    return { __value: rules, __count: rules.length };
  });
  step('세트 lookup', function() { return buildDashboardSetComponentLookup_(schedSheet.set); });
  step('결제 options', function() { return getTradePaymentOptions_(); });
  step('증빙 options', function() { return getTradeProofTypeOptions_(); });
  step('발행상태 options', function() { return getTradeIssueStatusOptions_(); });
  step('입금상태 options', function() { return getTradeDepositStatusOptions_(); });
  step('발행처 options', function() { return getTradeBillingCompanyOptions_(); });
  return profile;
}
