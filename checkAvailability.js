...
function checkSetHeaderAvail(sheet, row, 장비명, 수량, 반출일, 반출시간, 반납일, 반납시간, schedData, equipSheet) {
  ...
  if (overlaps.length === 0) {
    if (총보유 >= 수량) {
      ...
      sheet.getRange(row, 10).setValue("❌ 본체 가용" + 총보유 + " (보유" + 총보유 + ", " + 수량 + "개 요청 → 부족" + (수량 - 총보유) + ")");
...
function checkSingleRowWithData(sheet, row, reqID, 반출일, 반출시간, 반납일, 반납시간,
  ...
  if (overlaps.length === 0) {
    if (총보유 >= 수량) {
      ...
      sheet.getRange(row, 9).setValue("\u26A0\uFE0F 부족(가용" + 총보유 + "/" + 수량 + ")");
      sheet.getRange(row, 9, 1, 2).setBackground("#FFEB9C");
    }
