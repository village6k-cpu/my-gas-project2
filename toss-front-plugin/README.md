...
| `village-front/app.js` | 전체 흐름(조회→선택→결제→완료반영) |
...
## 폴더 구조 주의
`build-zip.sh`는 `village-front/` 폴더째로 압축합니다(공식 템플릿 방식).
업로드가 거부되면 폴더 안 파일들을 ZIP 최상위에 두는 방식으로 재시도:
```bash
cd village-front && zip -r ../village-front.zip . -x '*.DS_Store'
```
