...
create table if not exists village.trades (
  ...
  return_state_revision bigint not null default 0,
  payment_method      text,
...
-- 완료/해제 요청은 GAS가 발급한 단조 revision보다 새 요청일 때만 원자적으로 반영한다.
...
begin
  if p_kind not in ('setup', 'return') then raise exception 'invalid completion kind'; end if;
  if p_revision is null or p_revision <= 0 then raise exception 'invalid completion revision'; end if;
  select * into v_trade from village.trades where trade_id = p_trade_id for update;
  ...
  if p_kind = 'setup' and p_revision > v_trade.setup_state_revision then
    ...
    where trade_id = p_trade_id returning * into v_trade;
    v_applied := true;
