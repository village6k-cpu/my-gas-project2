...
async function resolveEquipment({
  ...
  if (!Array.isArray(queries) || queries.length === 0 || queries.length > MAX_EQUIPMENT) {
    throw new Error(
      `resolve input must be {"queries":["equipment term"]}; queries must contain 1-${MAX_EQUIPMENT} equipment terms`
    );
  }
...
function getCliHelpText() {
  return [
    'Usage: village-confirm-request.js <command> [--input-file PATH] [--env-file PATH]',
    'Read JSON from stdin unless --input-file is provided.',
    'Input envelopes:',
    '  resolve      {"queries":["equipment term"]}',
    '  create       {"request":{...}} or {...request fields...}',
    '  create-batch {"requests":[{...}]}',
    '  update       {"reqID":"RQ-YYMMDD-NNN","request":{...}}',
    ''
  ].join('\n');
}
