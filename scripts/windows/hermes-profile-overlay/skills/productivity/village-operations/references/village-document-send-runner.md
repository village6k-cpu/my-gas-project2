...
- Infer delivery authority from the whole current request and thread, not a fixed phrase table. When staff directly requests delivery, one recipient and one complete document version resolve, and validation finds no material ambiguity, generate, verify, and customer-send in the same turn. Preview/draft/check requests remain no-send.
- For registered-trade quote preview, resolve the trade in `my-gas-project2`, then use the 개고생2.0 GET preview endpoint (safe/no customer contact): `GET {DOCUMENT_API_URL}?action=previewQuote&key=***&id={거래ID}`. It returns `{tradeID,fileId,sheetUrl,pdfUrl}`.
  ...
  - On a preview-only request, report explicitly `고객 발송은 아직 안 했음` with 거래ID, period, subtotal, discount, final VAT-included total, and PDF attachment/link. On an authorized direct-delivery request, use the verified artifact and continue to the single send without asking again.
...
- For registered trade **statement** preview/send requests, use only registered-trade statement routes. Create and inspect the no-contact artifact with `GET previewStatement&key=...&id={거래ID}`, then, when the current request authorizes delivery, call `POST {action:"sendStatement", key, id, fileId}` in the same turn so the checked sheet is the one sent. A preview-only request stops before POST. Never call `previewQuote` or attach a quote PDF as a statement proxy.
...
- A semantically direct delivery request authorizes customer contact after unique resolution and validation; artifact generation is not a new approval gate. “만들어줘/확인해줘/찾아줘” remains no-send unless the full context clearly requests delivery.
