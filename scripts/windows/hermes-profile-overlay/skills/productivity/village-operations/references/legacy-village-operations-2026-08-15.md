...
> `27일 24:00` rolls into the next day and becomes `28일 00:00`.
...
2. Normalize date/time to API-friendly forms (`YYYY-MM-DD`, `HH:MM`) using current-year context when obvious.
   ...
   - **OBSOLETE incorrect rule:** `27일 24:00` keeps the written date and becomes `27일 00:00`. Current owner-confirmed behavior rolls `24시` / `24:00` into the next day: `27일 24:00` → `28일 00:00` before calling `insertAndCheckRequest`.
