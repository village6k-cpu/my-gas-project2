...
### 1. 작업 전 — 요구사항 정리와 즉시 조사
- 사용자의 요청은 내부적으로 항목별 정리하되, 실행 가능한 요청이면 안전한 읽기·조회·진단부터 즉시 시작할 것
- 이해한 범위와 진행 상황은 비차단형으로 알리고, 조사 시작 전 확인 답변을 요구하지 말 것
- 실제 근거를 확인한 뒤에도 돈·장비·수신자·일정·비가역 실행 결과를 바꾸는 모호성이 남을 때만 한 가지 핵심 질문을 할 것
- 넓은 범위나 많은 대상 자체를 모호성으로 취급하지 말고, 각 대상을 조회해 FACT/UNKNOWN/BLOCKED로 분류할 것
- "이 정도면 되겠지"라고 넘기거나 근거 없이 추측하지 말 것
...
- **myPage.js** — 고객용 내 예약 페이지 백엔드 (HMAC 토큰 검증, 이름 마스킹·연락처 미포함, 읽기 전용 — 변경/취소는 카톡 안내)
...
사용자는 아래 스크립트 이름이나 사용법을 기억하거나 직접 입력할 필요가 없다. Codex, Claude 등 이 저장소 지침을 읽는 AI가 대신 실행한다.

- 새 AI 세션에서 코드·문서·시트·배포처럼 저장소 상태를 바꾸는 작업을 받으면, AI의 첫 변경 작업 전에 반드시 `./scripts/startwork.sh`를 자동 실행한다.
- 사용자가 `작업 시작해`, `작업 시작하자`, `이어서 작업해`, `지금부터 고쳐`처럼 말하면 같은 시작 절차를 자동 실행한다. 사용자에게 명령어 입력을 요구하지 않는다.
- 검증까지 끝난 변경 작업은 사용자가 별도로 종료 명령을 기억하지 않아도 AI가 마무리한다. `main`에서는 변경 내용을 요약한 커밋 메시지를 스스로 만들어 `./scripts/endwork.sh "<변경 요약>"`를 실행한다. feature 브랜치에서는 `./scripts/finishbranch.sh "<변경 요약>"`를 실행하며 GAS를 배포하지 않는다.
- 사용자가 `작업 끝내줘`, `정리하고 종료해`, `커밋하고 푸시해`라고 말하면 위 종료 절차를 즉시 실행한다.
- 시작 스크립트가 `작업 시작 OK`를 출력하지 않거나 종료 스크립트가 중단되면, AI는 편집·배포를 계속하지 않고 원인을 사용자에게 보고한다.
- 정상적으로 도구 실행이 가능한데 사용자에게 `이 명령어를 직접 치세요`라고 떠넘기지 않는다.
