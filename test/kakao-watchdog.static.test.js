...
const watchdogPath = [
  process.env.VILLAGE_KAKAO_WATCHDOG_PATH,
  path.join(process.env.HOME || '', '.hermes/scripts/village_kakao_dom_watchdog.py'),
  '/Users/village6k/.hermes/scripts/village_kakao_dom_watchdog.py',
].filter(Boolean).find((candidate) => fs.existsSync(candidate));
const envPath = fs.existsSync(path.join(root, 'tools/kakao-dom-bridge/.env'))
  ? path.join(root, 'tools/kakao-dom-bridge/.env')
  : path.join(root, 'tools/kakao-dom-bridge/.env.example');
...
if (watchdogPath) {
  ...
  console.log('kakao watchdog external-file checks skipped: no local Hermes watchdog file');
}
