...
// 보류 행을 건너뛰어야 하고(최상위 미등록 품목은 자유입력으로 등록 가능, 모델 미선택만
// 일반 등록에서 차단), 재확인 시 과거 배너를 지워야 한다.
// ──────────────────────────────────────────────────────────────────────────
...
// 3-1) 최상위 "미등록 장비"는 오래전부터 허용한 자유입력 품목이다.
//      재고 목록에 없다는 이유만으로 등록 자체를 막아서는 안 된다.
assert.strictEqual(
  getBlockingRegisterIssue_([
    row('R1', '❓ 미등록 장비'),
  ], 'R1'),
  '',
  '최상위 미등록 장비는 일반 등록에서도 자유입력 품목으로 통과해야 한다'
);

// 3-2) 헤이빌리의 '바로 등록'은 사장님이 모델/재고 경고까지 확인한 최종 승인이다.
