...
const body = { scheduleId: 'BODY', name: '소니 FX9 바디', qty: 1, checkoutState: 'taken' };
const sdi = { scheduleId: 'SDI', name: 'SDI 롱라인', qty: 6, checkoutState: 'taken' };
...
test('같은 세트명으로 독립 반출된 대표행 둘을 모두 검증해 첫 행 누락을 숨기지 않는다', () => {
  const headers = [
    { scheduleId: 'A', name: '지선', setName: '지선', isSetHeader: true, qty: 1, checkoutState: 'taken' },
    { scheduleId: 'B', name: '지선', setName: '지선', isSetHeader: true, qty: 1, checkoutState: 'taken' },
  ];
  const blockers = status.returnCompletionBlockers(
