...
test('앱 저장 계층이 미완료 수량을 검사한 뒤에만 반납완료를 요청한다', () => {
  ...
  assert.match(store, /flushTradePersist\(tradeId\)[\s\S]{0,1000}gasMutation\("toggleReturn"/);
...
test('상세 반납 수량을 내구 저장하기 전에는 GAS 완료 증거를 만들지 않는다', () => {
  const store = read('apps/today-dashboard/lib/data/store.ts');
  const start = store.indexOf('export async function setReturnCount');
  const end = store.indexOf('\n// ── 결제', start);
  const fn = store.slice(start, end);
  const incomplete = fn.indexOf('writeback === false');
  const complete = fn.indexOf('writeback === true');
  assert.ok(incomplete >= 0 && complete > incomplete);
  assert.ok(fn.indexOf('await gasMutation', incomplete) < fn.indexOf('await flushTradePersist', incomplete));
  assert.ok(fn.indexOf('await flushTradePersist', complete) < fn.indexOf('await gasMutation', complete));
  assert.match(fn, /returnCountWriteGenerations/);
  assert.match(fn, /returnCountWriteChains/);
});
