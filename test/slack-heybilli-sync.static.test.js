...
assert(server.includes('rawContextHasVisionEvidence') && server.includes('raw_context: changed || rawContextHasVisionEvidence(rawContext) ? rawContext : previous?.raw_context ?? rawContext'), 'vision enrichment must be stored once and preserved on later plain rescans');
...
assert(remote.includes('delete structural.actual_name') && remote.includes('delete structural.actual_taken_qty'), 'stale browsers must not erase server-owned correction overlays');
