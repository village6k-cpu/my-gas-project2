...
import { effectiveRegisteredOperationReceipts } from './registered-operation-recovery.mjs';
import {
...
- A customer can explicitly correct a phone already stored on the exact pending RQ. Include customer_identity_update with the live expected_name/expected_phone and the corrected name/phone, quoting the actual customer correction in source_evidence. A nonempty old phone is not a blocker. GAS verifies that the baseline still matches under lock; never overwrite a changed baseline or borrow a number from another customer.
...
- Reconcile ALL still-unfulfilled requests for the same booking across the full conversation and current schedule before constructing a registered change. A short staff reply may approve several preceding customer requests; determine its scope semantically, not just from the last bubble. A resent full list does not remove previously approved equipment without explicit removal evidence. Keep already-applied additions and include only the missing approved delta.
- Link request_id only when the pending RQ's entire top-level equipment plan equals this operation's exact delta. A related customer/date is insufficient. For an unrelated or partially applied RQ, omit request_id; do not duplicate already-applied items or drop the other outstanding additions to force a match.
- A registered rejection with error.details.code=REGISTERED_CORRECTION_PREFLIGHT_REJECTED and noMutationPerformed=true proves this attempt changed nothing. Use the reason plus fresh village_read to correct the typed input yourself, then call the registered tool again within this turn (at most two input corrections). The same payload only returns its old receipt; change it only with grounded evidence. Do not turn an input error into a routine owner task when you can resolve it.
- Any other blocked/failed result, partial_success, contradictory readback, or uncertain transport remains no-send owner review; never replay an uncertain or already-applied operation. Successful final reconciliation must match the corrected operation and current schedule.
...
- 모든 신규 고객 장비 문의는 일정·연락처 완성도나 직원 답변 여부와 무관하게 should_write_to_sheet=true로 확인요청에 먼저 남긴다. 모르는 일정은 빈칸이며 재고 가능은 단정하지 않는다.
...
- RECENT_BOT_SENDS에 없는 빌리지측 버블은 사장(사람)의 수동 응대다. 이미 답한 내용은 반복·번복하지 않는다. 확정·감사 인사가 미해결 필수 정보를 채운 것은 아니다. 대화·원장·정책에도 없는 정보는 고객에게 한 번 묻고 답변으로 기존 승인/RQ의 등록을 완성한다. 내부 확인 카드만 남기고 종료하지 않는다.
...
14. 미실행·거절된 수정안은 customer_requested_pending_revision/staff_confirmed_registration/staff_confirmed_mutation에 남기지 말고 null로 둔다. 유효한 실행 요청 또는 정확히 대응하는 성공 영수증만 이 필드에 허용한다. 실패 사실·다음 조치는 owner_case/follow_up_items에 보고한다.
15. Return only the final machine-readable JSON below.

FINAL OUTPUT FORMAT:
Print a line containing FINAL_JSON, then a fenced json object.
owner_case의 다섯 필드는 모두 공백 아닌 문자열이어야 한다. 해결됐으면 problemSummary에 검증 결과를 적고 follow_up_items=[], shouldCreateTask=false로 둔다. 남은 실무만 후속조치로 유지한다. requiresHumanAction=true의 actionFamily는 invoice_issue, reservation_change, payment_reconcile, inventory_check, document_approval 중 하나이며 none은 금지다.
The JSON schema:
{
...
export function validateAiDecisionContract(decision = {}, options = {}) {
  ...
  if (decision.should_write_to_sheet === true) {
    ...
    for (const [field, validator, expected] of [
      ...
      ['end_date', isStrictIsoDate, 'YYYY-MM-DD'],
      ['pickup_time', isStrictConfirmRequestTime, 'HH:MM'],
      ['return_time', isStrictConfirmRequestTime, 'HH:MM']
    ]) {
      const value = text(row[field]).trim();
      if ((!incompleteNewInquiry || value) && !validator(value)) {
...
function isIncompleteInquiryCapture(decision = {}, row = {}) {
  ...
  const existingIds = Array.isArray(decision.existing_confirm_request_ids)
    ? decision.existing_confirm_request_ids.filter((value) => text(value).trim())
    : [];
  const mode = text(row.equipment_write_mode).trim();
  const hasMissingScheduleField = [row.start_date, row.pickup_time, row.end_date, row.return_time]
...
function partialConfirmRequestWindowForSheet(row = {}) {
  const raw = {
    start_date: text(row.start_date).trim(),
    pickup_time: text(row.pickup_time).trim(),
    end_date: text(row.end_date).trim(),
    return_time: text(row.return_time).trim()
  };
  if (raw.start_date && !strictConfirmRequestDate(raw.start_date)) return null;
  if (raw.end_date && !strictConfirmRequestDate(raw.end_date)) return null;
  const pickup = raw.pickup_time ? confirmRequestTimeParts(raw.pickup_time) : null;
  const returned = raw.return_time ? confirmRequestTimeParts(raw.return_time) : null;
  if ((raw.pickup_time && (!pickup || pickup.hour === 24)) || (raw.return_time && !returned)) return null;

  const normalized = {
    start_date: raw.start_date,
    pickup_time: pickup ? `${String(pickup.hour).padStart(2, '0')}:00` : '',
    end_date: raw.end_date,
    return_time: ''
  };
  if (returned) {
    if (returned.hour === 24) {
      if (!normalized.end_date) return null;
      normalized.end_date = addDaysToYmd(normalized.end_date, 1);
      normalized.return_time = '00:00';
    } else if (returned.minute > 0 && returned.hour < 23) {
      normalized.return_time = `${String(returned.hour + 1).padStart(2, '0')}:00`;
    } else if (returned.minute > 0 && normalized.end_date) {
      normalized.end_date = addDaysToYmd(normalized.end_date, 1);
      normalized.return_time = '00:00';
    } else {
      normalized.return_time = `${String(returned.hour).padStart(2, '0')}:${String(returned.minute).padStart(2, '0')}`;
    }
  }
  return normalized;
}
...
const INQUIRY_POINTER_TEXT = '카톡 채널 관리자에서 확인하세요';

function autoReplySent(row = {}) {
  return row?.payload?.auto_reply_sent === true;
}

function inquiryPointerLine(row = {}) {
  return `${autoReplySent(row) ? '✅ AI 자동응답 보냄' : '📩 AI 응답 없음'} · ${INQUIRY_POINTER_TEXT}`;
}
...
export function buildSlackInquiryMessage(row = {}, options = {}) {
  ...
  const blocks = [
    ...
    { type: 'context', elements: [{ type: 'mrkdwn', text: inquiryPointerLine(row) }] },
    {
      type: 'actions',
      elements: [
  ...
  return {
    channel: route.channel,
    text: truncateSlackFallbackText(`${safeSlackCustomerName(row)} · ${inquiryLabel(row)} · ${autoReplySent(row) ? 'AI 응답됨' : '카톡 확인'}`, 40),
    blocks
  };
}
...
export async function prepareKakaoGatewayDecision({
  ...
  if (exactTurn) trustedToolReceipts = effectiveRegisteredOperationReceipts(trustedToolReceipts,
    (receipt) => exactTrustedRegisteredReservationChangeReceipt(receipt, { jobId, roomKey, roomRevision }));
