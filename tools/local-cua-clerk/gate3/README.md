...
- 대표가 `@맥에이전트` 뒤에 쓴 자연어 한 건을 그대로 새 persisted Codex 작업으로 처리하는 범용 경로 구현
...
- 사용자는 채널에 정확히 `맥에이전트 상태 확인`을 입력한다. 기존 직접 멘션 방식은
  호환용으로만 유지한다.
- 범용 로컬 업무는 대표가 Slack에서 `@맥에이전트 <자연어 업무>`로 직접 요청한다. 이 원문이
  실행 지시가 되므로 Hermes 스킬·AX2 프로필·HeyBilly의 권한 변환이 필요하지 않다. 최상위
  요청이면 그 메시지에 새 스레드를 만들고, 기존 스레드에서 요청하면 같은 스레드를 유지한다.
...
- HeyBilly의 기존 `<@맥에이전트> 작업 요청` 인계도 호환용으로 수용하지만, 범용 실행 권한의
  기준은 대표의 직접 멘션이다. 코드펜스·숨은 필드·빈 본문은 HeyBilly 범용 경로로 승격하지
  않으며, task 이름에는 요청 ID만 넣는다. 자연어 원문은 승인된 Codex 작업 기록에는 남지만
  로컬 원장·서비스 로그·ACK에는 복제하지 않는다.
