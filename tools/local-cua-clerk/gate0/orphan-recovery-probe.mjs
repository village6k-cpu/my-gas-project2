...
export function isGrantUsable(grant, activeEpoch, usedGrants = new Set()) { return Boolean(grant?.epoch && grant?.grantId && grant.epoch === activeEpoch && !revokedEpochs.has(grant.epoch) && !usedGrants.has(grant.grantId)); }
...
export async function recoverOwnedOrphan({ orphan, grant, activeEpoch, usedGrants = new Set(), identityReader, signal = () => true, isAlive = () => true }) {
  if (!isGrantUsable(grant, activeEpoch, usedGrants)) return { result: 'BLOCKED', reason: revokedEpochs.has(grant?.epoch) ? 'grant_reused' : grant?.epoch !== activeEpoch ? 'wrong_epoch' : 'grant_reused', signals: [] };
...
export async function runOrphanRecoveryProbe({ spawnImpl = nodeSpawn, identityReader = defaultIdentityReader, signalImpl = process.kill, childFactory, now = () => new Date().toISOString(), runId } = {}) {
  ...
  let identity;
  try { identity = await identityReader(pid); } catch { return makeProbe({ probeId: 'orphan_recovery', result: 'BLOCKED', checkedAt: now(), runId, evidence: { status: 'unknown', criterion: 'identity', pointer: 'capture_failed' }, errorClass: 'identity_mismatch' }); }
