...
export function hermesPrompt(result, config) {
  ...
  return [
    ...
    '먼저 독립 실물 대여품인지, 세트 동봉품인지, 여러 실물을 묶어 적은 행인지 판단하세요. 세트 원문과 inventoryPolicy를 함께 읽습니다. 케이블·기본 부속·포장 구성과 정책상 재고 검사 제외인 메모리/배터리는 총보유수량을 묻는 대상이 아닙니다. 여러 실물을 한 셀에 적었다면 실제 재고 항목들로 나눠 계산하고, 상위 세트와 중복 차감하지 않습니다. 구성품 여부나 명확한 기존 실물 연결처럼 자료로 판단할 수 있는 일을 대표님에게 되묻지 마세요.',
    '세트별 판단은 review-stock에 {action:"classify_component",reason,resolutions:[{setName,componentName,sourceHash,disposition:"included" 또는 "stock",allocations:[]}]}를 전달합니다. setName/componentName/sourceHash는 stockCatalog.semanticScopes의 해당 항목 그대로입니다. included는 동봉품·정책 제외여서 allocations=[]; stock은 [{equipmentId,equipmentName,quantity}]로 한 원문 구성품 단위당 실제 장비 배정을 적습니다. 한 항목에 여러 실물을 지정할 수 있습니다. 이름 고정 분기나 전역 별칭을 만들지 않고 해당 세트 문맥에만 반영됩니다. dry-run 후 --write, verified/effective 확인. 종전 waiting_owner 질문이 있어도 답변이 필요 없는 동봉품이면 스스로 분류를 완료하세요. 기존 질문을 중복 발송하지 않습니다.',
