...
- 거래ID가 없으면 직원이 입력한 고객명이 카드 고객명과 정확히 같고, 반출/반납 단계와 예정일도 맞는 후보 하나만 선택한다. 부분일치는 자동 반영하지 않는다.
...
- 헤이빌리에 기존 거래 카드가 없으면 GAS 검색 결과를 가져오거나 새 카드를 만들지 않는다. 같은 스레드에 거래ID/정확한 대여자명을 요청한다.
