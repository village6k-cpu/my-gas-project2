...
4. 터미널에서는 아래 CLI의 `lookup`, `lookup-equipment`, `record-equipment`, `scan-stock`, `confirm-stock`, `apply`, `ask`, `ignore`만 사용한다. 다른 코드·파일·외부 시스템을 변경하지 않는다.
...
## 카탈로그에만 있는 장비의 실재고 확인

대표가 재고 보고 스레드에 답하면 stockQuestions로 수집된다. 이 경로는 거래번호를 요구하지 않는다.
AI가 전체 스레드와 카탈로그를 읽고 어떤 장비의 실재고인지 해석한다. 서버는 숫자나 이름 키워드로 의미를 추측하지 않고 소유자·원문·현재 보고·기존 장비 중복을 검증한다.

- scan-stock은 정확한 재고 보고 스레드, ownerReplies, sourceHash, 전체 장비, 카탈로그를 준다.
- confirm-stock 입력: {reportId,confirmation:{catalogName,stockTotal,stockMaintenance,category,major,sourceMessageTs,quote,sourceHash}}.
- quote는 대표님 답변 전체다. 숫자를 여러 장비에 연결할 때 AI가 문맥으로 판단하고 원문은 그대로 보존한다.
- 먼저 --write 없이 확인하고, LIVE에서는 --write로 신규 원장과 장비마스터를 반영한다. 이 업무에 대한 대표 승인으로 해당 댓글의 실재고 등록은 허용된다.
- 실재고 수량만 답하고 수리 언급이 없으면 신규 등록 기본 정비수량은 0이다. 이 기본값을 결과에 명시한다. 수리·미확인 언급이 있으면 숨기지 않는다.
- stockTotal은 실보유수량이다. 대여 이력·예약 수량·외부 조달 수량으로 추정하지 않는다.
