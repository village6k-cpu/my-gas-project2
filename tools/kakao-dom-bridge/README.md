...
이 테스트 스크립트는 기본 debounce를 5초로 줄이고, debounce 완료 job을 `tools/ai-browser-worker/run.sh`로 넘깁니다. 이 worker는 아직 판단하지 않고 job JSON을 `queue/ai-worker-inbox/`에 저장만 합니다.
