create table village.inventory_stock_confirmations (
...
alter table village.inventory_stock_confirmations enable row level security;
revoke all on village.inventory_stock_confirmations from anon, authenticated;
grant select,insert,update on village.inventory_stock_confirmations to service_role;

create function village.confirm_missing_inventory_stock(p_source_key text,p_item jsonb,p_evidence jsonb)
...
begin
 ...
 if exists(select 1 from village.inventory_audit_sessions where mode='full_shop' and status='draft') then raise exception 'inventory_audit_in_progress'; end if;
 ...
 from village.equipment_ledger where equipment_id ~ ('^'||v_prefix||'-[0-9]+$');
