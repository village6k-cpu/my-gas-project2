...
4. Optionally start the Windows-owned Hermes gateway. A normal bridge, gateway, restart, or watchdog start validates the existing `kakaoworker` profile but never imports or replaces its skill tree. The live profile owns its native Hermes learning. Use `sync-hermes-profile-overlay.ps1` only as an explicit migration/recovery command after a verified backup and conflict review; execution additionally requires `-ConfirmProfileSkillTreeReplacement`, while `-WhatIf` remains available for inspection.
...
## Native Kakao Hermes Gateway cutover

The native Gateway transport is a separate, benchmark-gated transition. Do not treat the offline replay as a latency pass. `docs/kakao-hermes-gateway-benchmark-report.json` must contain a provider-backed, comparable run with at least 20 measured requests for each arm, `accepted=true`, and `latency_status=pass`. The checked-in offline structural report is intentionally `accepted=false`; while it remains blocked, every apply path refuses before any health probe, task change, process restart, or customer-facing action.

Use the following sequence after Windows already owns the live Kakao bridge:

1. Install the reviewed profile-scoped plugin with `sync-kakao-hermes-plugin.ps1`, then register the disabled profile-scoped task with `register-hermes-gateway-tasks.ps1` and confirm its plan says root `mutated=false`. The install receipt must remain at `%LOCALAPPDATA%\hermes\profiles\kakaoworker\plugin-state\kakao_village.json`. Startup hashes every installed manifest file against that receipt and validates that `Hermes_Gateway_Kakaoworker_Native` uses the reviewed launcher, exact Hermes venv, and exact environment file; a receipt or task name alone is not proof. The legacy `Hermes_Gateway_Kakaoworker` task is retained unchanged because its existing ACL may require elevation and its VBS is retired.
2. Run the native no-send replay and a provider-backed A/B benchmark using the exact `grok-4.5`, `xai-oauth`, `xhigh`, 90-turn model contract. Preserve a smoke-evidence JSON showing `nativeSessionResult=pass`, schedule owner review required, zero Kakao sends, zero Slack sends, zero GAS writes, and the observed kill switch.
3. Render the transition without touching the runtime: `start-kakao-live.ps1 ... -BenchmarkReportPath ... -PluginReceiptPath ... -SmokeEvidencePath ... -ConfirmKakaoGatewayCutover -PlanOnly`.
4. Only after the report passes, run the same command without `-PlanOnly`. The command requires an idle bridge, direct CDP `authenticated=true` and `watcherReady=true`, a matching plugin manifest, and the exact model contract. It restarts only the owned Kakao bridge as `gateway`, starts only `Hermes_Gateway_Kakaoworker_Native`, and leaves the root Slack Gateway and healthy Chrome untouched.
5. Accept success only when `/health` directly reports transport `gateway`, a fresh consumer, queue `ready=claimed=retry=failed=0`, zero unnotified application failures, schedule owner review and kill-switch policy enabled, and the exact profile/PID/plugin receipt also verifies. A port or HTTP 200 alone is not success.
6. Register post-cutover tasks only with both `-ConfirmProductionOwnership` and `-ConfirmKakaoGatewayCutover`, plus the accepted benchmark, plugin receipt, and smoke paths. The logon task uses `-GatewayMaintenance`; it does not rerun the initial handoff. The watchdog repairs only `Hermes_Gateway_Kakaoworker_Native` and never targets the root Slack Gateway.

Rollback is explicit and profile-scoped: render it first with `start-kakao-live.ps1 ... -RollbackToCli -PlanOnly`, then run it with both `-RollbackToCli` and `-ConfirmKakaoGatewayCutover`. It stops and disables only `Hermes_Gateway_Kakaoworker_Native`, switches only the owned Kakao bridge back to `cli`, and preserves the root Slack Gateway and healthy Chrome. Never run this rollback while an unresolved durable confirmation operation or active bridge job exists.
