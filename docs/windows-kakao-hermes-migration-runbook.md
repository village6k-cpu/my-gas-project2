...
1. Install the reviewed profile-scoped plugin with `sync-kakao-hermes-plugin.ps1`, then register the disabled profile-scoped task with `register-hermes-gateway-tasks.ps1` and confirm its plan says root `mutated=false`. The install receipt must remain at `%LOCALAPPDATA%\hermes\profiles\kakaoworker\plugin-state\kakao_village.json`. Startup hashes every installed manifest file against that receipt and validates that `Hermes_Gateway_Kakaoworker` uses the reviewed launcher, exact Hermes venv, and exact environment file; a receipt or task name alone is not proof.
