...
> Claude 8-에이전트 병렬 분석 산출물. 2026-06-08 시점의 역사 스냅샷이며
> 현재 운영 사실은 live Sheet/GAS/API와 owner-confirmed 정책을 우선한다.
> 2026-08-18 확인: 대여일수 유예는 +3시간, 등록완료 알림톡은 자동발송이다.
...
| **팝빌 / 카카오 알림톡** | 반출·반납 안내톡과 등록완료 알림톡 자동발송. 가용확인 알림톡은 별도 승인 경계 | 현재 live GAS 및 2026-08-18 owner-confirmed 정책 |

**핵심 비즈니스 규칙(코드 검증):**
- **장기할인 7티어**: 1일0%/2일10%/3~5일20%/6~9일35%/10~14일40%/15~19일45%/20일+50%. 현재 일수 정책은 24시간 1일, +3시간 동일, 3시간 초과 +1일.
...
9. **외부 연동 & 알림 (Integrations & Notifications)** — 개고생2.0(회계), 팝빌 알림톡, 장비리스크 백엔드, LLM 파서를 어댑터 패턴으로 격리. 위치 하드코딩(D→E,M→C) 제거. 등록완료 자동발송은 유지하고 가용확인 발송은 별도 승인 경계로 둔다. 동기 UrlFetch를 비동기 큐로 분리해 등록 속도 보호
